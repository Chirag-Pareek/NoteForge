...
class NotesRepository {
  ...
  Stream<List<SubjectModel>> watchSubjects() {
    return _subjectsRef
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map((d) => SubjectModel.fromDocument(d)).toList());
  }

  Future<SubjectModel> addSubject(String name, String color) async {
    final now = DateTime.now();
    final doc = await _subjectsRef.add(SubjectModel(
      id: '',
      userId: _uid,
      name: name,
      chaptersCount: 0,
  ...
  Future<void> updateSubject(String id, {String? name, String? color}) {
  ...
  Future<void> _updateSubjectChaptersCount(String subjectId) async {
    final count = (await _chaptersRef
  ...
  Stream<List<ChapterModel>> watchChapters(String subjectId) {
    return _chaptersRef
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map((d) => ChapterModel.fromDocument(d)).toList());
  }

  Future<ChapterModel> addChapter(String subjectId, String name) async {
    final now = DateTime.now();
    final doc = await _chaptersRef.add(ChapterModel(
      id: '',
      subjectId: subjectId,
      name: name,
      topicsCount: 0,
    ...
    final snapshot = await doc.get();
    return ChapterModel.fromDocument(
        snapshot);
  }
  ...
  Stream<List<TopicModel>> watchTopics(String chapterId) {
    return _topicsRef
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map((d) => TopicModel.fromDocument(d)).toList());
  }

  Future<TopicModel> addTopic(
      String chapterId, String subjectId, String name) async {
    final now = DateTime.now();
    final doc = await _topicsRef.add(TopicModel(
      id: '',
      chapterId: chapterId,
      subjectId: subjectId,
      name: name,
  ...
  Future<void> updateNote(String id, {String? title, String? content, bool? isDraft}) {
    final data = <String, dynamic>{
      'updatedAt': FieldValue.serverTimestamp(),
    };
